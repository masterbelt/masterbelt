...
// lowerLiteral lowers a Literal node (its single Int/BinInt/OctInt/HexInt/
// String/Datetime/Duration/True/False/Null leaf) to the matching literal
// expression. The radix leaf kinds carry their base into the IntLit so a
// formatter can recover the spelling without re-parsing the text.
func lowerLiteral(t cst.Tree, buf source.Buffer, node *cst.Node) ast.Expr {
	switch literalKind(t) {
	case token.Int:
		return ast.NewIntLit(t.Text(buf), ast.BaseDecimal, node)
	case token.BinInt:
		return ast.NewIntLit(t.Text(buf), ast.BaseBinary, node)
	case token.OctInt:
		return ast.NewIntLit(t.Text(buf), ast.BaseOctal, node)
	case token.HexInt:
		return ast.NewIntLit(t.Text(buf), ast.BaseHex, node)
	case token.String:
