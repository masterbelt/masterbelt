...
var allowedImports = map[string]bool{
	...
	"github.com/masterbelt/masterbelt/pkg/belt/builtin": true,
	"github.com/masterbelt/masterbelt/pkg/belt/types":   true,
}
...
// this package leans on (pkg/belt/types, its infer subpackage included)
...
func TestTypeAlgebraSyntaxFree(t *testing.T) {
	for path := range packageImports(t, "../types") {
		if strings.Contains(path, "/pkg/source/") && !strings.HasSuffix(path, "/pkg/source/ir") {
			t.Errorf("pkg/belt/types imports %q — the type algebra must stay syntax-free", path)
		}
	}
	for path := range packageImports(t, "../types/infer") {
		if strings.Contains(path, "/pkg/source/") &&
			...
			t.Errorf("pkg/belt/types/infer imports %q — the checker reads the AST and the IR, nothing beneath", path)
		}
	}
}
