...
func graphLet(s *ir.Let, ctx graphCtx, scope *graphScope) bool {
	...
	// A relation value (let m = self.where(...)) folds and binds like any other value
	// — it is an ir.ConstRelation carrying its chain — so a use, a reassignment, or a
	// capture of it goes through the ordinary local path. The where it was built from
	// folded its captured scalars at that point, so the bound relation holds the values
	// they had then, the way a closure captures its locals at creation.
...
func graphIteration(s *ir.For, elem *ir.Constant, ctx graphCtx) (*ir.Constant, graphOutcome) {
	...
	defer scope.restore()
	if s.Var != "" && elem != nil {
		// The loop variable shadows any outer binding of the same name — including a
		// relation, now an ordinary local — for the body, restored on exit by the scope.
...
// tracking (the graph carries its types on the nodes). A relation-valued let is an
// ordinary local (an ir.ConstRelation), so it needs no special tracking.
