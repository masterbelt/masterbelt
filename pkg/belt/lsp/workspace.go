...
// exprType is the type the checker settled for an expression node — its
// typed-value-graph entry — or ir.Invalid when the checker typed it with no usable
// type (a master in a constant initializer, a name a local shadows, an unresolved
// form). It is the editor's read of the checker's own typing, the single source of
// truth a member-access receiver resolves through (a body's master-as-relation, a
// relation chain's result) instead of the editor re-deriving the scope rules.
func (v view) exprType(e ast.Expr) ir.Type {
	if t, ok := v.ws.prog.ExprTypes(v.id)[e]; ok {
		return t
	}
	return ir.Invalid
}
