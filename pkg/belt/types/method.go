...
func MethodResult(reg *builtin.Registry, recv ir.Type, method string, args []ir.Type) ir.Type {
	...
	sel := matches[0]
	if _, bound := MethodBoundViolation(reg, sel.Method, sel.Subst); bound != nil {
		return ir.Invalid
	}
...
//
// The bound is substituted by the solution before the check, so a bound that names
// another solved variable — accept<T: Box<U>> on Host<int> — is verified as Box<int>
// rather than the unresolved Box<U>. A solution that is itself a type variable is
// checked through its own bound (Satisfies maps a TypeVar to its bound), so a generic
// call h.take(u) with an unbounded u is rejected rather than silently accepted.
