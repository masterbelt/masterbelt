...
func TestGenerateSample(t *testing.T) {
	got, err := Generate("testdata/sample", config{markers: []string{"Node"}, roots: []string{"Root"}})
	if err != nil {
...
func TestGenerateRejects(t *testing.T) {
	if _, err := Generate("testdata/sample", config{markers: []string{"Bogus"}, roots: []string{"Root"}}); err == nil {
		t.Error("unknown marker accepted")
	}
	if _, err := Generate("testdata/sample", config{markers: []string{"Node"}, roots: []string{"Bogus"}}); err == nil {
