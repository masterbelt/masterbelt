...
func TestRefinementSatisfied(t *testing.T) {
	m, diags := analyze("pub type Port = int32 where self >= 1 && self <= 65535\n" +
...
func TestRefinementViolation(t *testing.T) {
	...
	_, diags := analyze("pub type Port = int32 where self >= 1 && self <= 65535\n" +
...
func TestRefinementViolationNegative(t *testing.T) {
	_, diags := analyze("pub type Level = int8 where self >= 0 && self <= 100\n" +
		"const Neg: Level = -1\n")
	if got := codes(diags); len(got) != 1 || got[0] != CodeRefinementViolation {
		t.Fatalf("codes = %v, want [refinement_violation]", got)
	}
...
func TestRefinementViolationThroughReference(t *testing.T) {
	...
	_, diags := analyze("pub type Port = int32 where self >= 1\n" +
		"const A = 0\nconst B: Port = A\n")
	if got := codes(diags); len(got) != 1 || got[0] != CodeRefinementViolation {
		t.Fatalf("codes = %v, want [refinement_violation]", got)
	}
...
func TestRefinementNotBool(t *testing.T) {
	m, diags := analyze("pub type Bad = int8 where self + 1\nconst c: Bad = 1\n")
...
func TestRefinementNotConstant(t *testing.T) {
	...
	_, diags := analyze("pub type Bad = int8 where self / 0 == 0\nconst c: Bad = 1\n")
...
func TestRefinementBadMethod(t *testing.T) {
	...
	_, diags := analyze("pub type Bad = int8 where self.bogus()\nconst c: Bad = 1\n")
...
func TestRefinementWithImpl(t *testing.T) {
	...
	// annotated constants.
	_, diags := analyze("pub type Pct = int8 where self >= 0 && self <= 100 impl {\n" +
		"  pub increment(): self {\n    return self + 1\n  }\n}\n" +
		"const p: Pct = 101\n")
	if got := codes(diags); len(got) != 1 || got[0] != CodeRefinementViolation {
		t.Fatalf("codes = %v, want [refinement_violation]", got)
