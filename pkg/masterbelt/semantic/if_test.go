...
func TestIfConditionNotBool(t *testing.T) {
	_, diags := analyze("pub fn f(n: nint): nint {\n  if n {\n    return 1\n  }\n  return 0\n}\n")
...
func TestIfExhaustiveReturns(t *testing.T) {
	_, diags := analyze("pub fn sign(n: nint): nint {\n  if n > 0 {\n    return 1\n  } else {\n    return -1\n  }\n}\n")
	if len(diags) != 0 {
		t.Fatalf("unexpected diagnostics: %v", codes(diags))
	}
}
