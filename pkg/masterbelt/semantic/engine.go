...
type database struct {
	...
	reused         map[queryKey]bool // keys served from a verified memo since the last setInput (D-1 M-reuse side-channel)
}
...
func (db *database) demand(key queryKey) any {
	...
	if m != nil && db.verify(m) {
		...
		db.reused[key] = true // a memo carried across the revision: early cutoff worked
		return m.value
	}
